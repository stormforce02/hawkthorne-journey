...
function Baseball:draw()
    ...
    elseif self.held then --ugly hack to fix baseball
        love.graphics.drawq(self.image, love.graphics.newQuad( 0, 0, 9, 9, 18, 9 ),
                            math.floor(self.position.x), 193)
    else
        love.graphics.drawq(self.image, love.graphics.newQuad( 0, 0, 9, 9, 18, 9 ),
                            math.floor(self.position.x), math.floor(self.position.y))
    end
end
...
function Baseball:update(dt, player)
    ...
    if self.thrown then
        ...
        if self.thrown and self.position.y > self.floor then
            ...
            if math.abs(current_y_velocity) < 30 then
                --stop bounce
                self.velocity.y = 0
                self.thrown = false
            else
                --bounce 
                self.velocity.y = -.75 * math.abs( current_y_velocity )
            end
        end
